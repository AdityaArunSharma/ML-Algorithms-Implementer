...
class PolynomialRegression:
    ...
    def main(self,dataPath):
        ...
        enterDegree_label = Label(app,text="Enter degree")
        yPosition+=40
        enterDegree_label.place(x=width/12,y=yPosition,height=15,width=150)

        inputDegree = Text(app,height=15,width=100)
        yPosition+=30
        inputDegree.place(x=width/12,y=yPosition,width=100,height=20)
        ...
        def predictFutureButtonClicked(event=None):
            ...
            if(not RepresentsInt(inputDegree.get("1.0", "end-1c"))):
                tkinter.messagebox.showinfo('Error!','Enter a valid degree')
                return
        ...
        def performPolynomialRegression():
            ...
            polynom = PolynomialFeatures(degree=int(inputDegree.get("1.0", "end-1c")))
            ...
            Y=y[:50]
            Y_ = PolyReg.predict(polynom.fit_transform(sc_x.fit_transform(x[:50])))
            plt.scatter(Y,Y_)
        ...
        app.title("Polynomial Regression")
        app.mainloop()
